...
if __name__ == '__main__':
    ...
    with open('portraitString12.json','r') as file:
        portraitString12 = json.load(file)

    with open('portraitIndexOfCC12.json','r') as file:
        portraitIndexOfCC12 = json.load(file)
        
    tempRet = []
    #keys = list(subSampleCC.keys())
    keys = portraitString12
    ...
    for v in [5,10,20]:
        ...
        for k in keys[lower:upper]:
            index = portraitString12.index(k)
            heavisideIndex = portraitIndexOfCC12[index]
            ...
            #for key in keys:
                ...
                samples = regionSample(theta2,theta1,upperBound)
                portrait = dict()
                #equilibrium = dict()
